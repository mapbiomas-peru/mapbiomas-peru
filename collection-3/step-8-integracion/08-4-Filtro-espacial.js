...
param.codesAndVersions.forEach(function(codeAndVersion) {
  ...
  var code_region = codeAndVersion[0]
  var version_input = codeAndVersion[1];
  var version_output = param.version_output;

  var prefixo_out = param.pais+ '-' + code_region + '-' + version_output

  // ------------------------
  
  var region = ee.FeatureCollection(regionesclass)
                    .filterMetadata("id_regionC","equals", code_region);
  ...
  var mosaicRegion = code_region.toString().slice(0, 3);
  ...
  var class_input = ee.ImageCollection(dirinput)
                 .filterMetadata('code_region', 'equals', code_region)
  ...
  // Add bands mask
    ...
    var bandsOccurrence = ee.Dictionary(
        bandNames.cat(class_input.bandNames()).reduce(ee.Reducer.frequencyHistogram())
    );
        ...
        function (key, value) {
            return ee.Image(
                ee.Algorithms.If(
                    ...
                    class_input.select([key]).byte(),
                    ee.Image(27).rename([key]).byte().updateMask(class_input.select(0))
                )
            );
        }
    );
    ...
    class_input = imageAllBands
  
  
  
  
  // add connected pixels bands
  var imageFilledConnected = class_input.addBands(
      class_input
  ...
  class_input= imageFilledConnected;
  ...
  // Map.addLayer(class_input.select(bandNames), vis, 'class_input_1985', false);
  
  var ano = '1985'
  var moda_85 = class_input.select('classification_'+ano).focal_mode(1, 'square', 'pixels')
  moda_85 = moda_85.mask(class_input.select('classification_'+ano+ '_connected').lte(min_connect_pixel))
  var class_outTotal = class_input.select('classification_'+ano).blend(moda_85)
  ...
  for (var i_ano=0;i_ano<anos.length; i_ano++){  
    var ano = anos[i_ano]; 
    var moda = class_input.select('classification_'+ano).focal_mode(1, 'square', 'pixels')
    moda = moda.mask(class_input.select('classification_'+ano+ '_connected').lte(min_connect_pixel))
    var class_out = class_input.select('classification_'+ano).blend(moda)
  ...
  if(param.exclusion.clases.length>0){
     var clasifi = ee.List([])
        class_input = class_input.select(bandNames)
        param.exclusion.clases.forEach(function(clase){
          var clasif_code =class_input.eq(clase).selfMask()
          clasifi = clasifi.add(class_input.updateMask(clasif_code).selfMask())
        })
  ...
  if(param.exclusion.years.length>0){
    var yearExlud = class_input.select(bandNamesExclude);  //addbands
  ...
  if(param.piramideActive) {
    ...
    Map.addLayer(class_input.select(bandNames), vis3, 'class-ORIGINAL'+param.year);
    ...
    Map.addLayer(class_input.select(bandNames)
...
//   var area0 = enmascararValor(class_input, nombreBanda).reduceRegion({
//     reducer: ee.Reducer.sum(),
//     geometry: region.geometry(),
