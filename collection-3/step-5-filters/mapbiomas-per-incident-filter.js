...
var param = {
    ...
    version_input_class:22, //  Clasificacion de entrada (ojo no es el paso 13-1)
    version_output:23, //    Salida 
...
var classification_corregida = classification_input
...
classification_corregida = classification_corregida.blend(corrige_borda)

Map.addLayer(classification_corregida.reproject('EPSG:4326', null, 30), vis, 'class_FF_corregida'+param.year);

classification_corregida =classification_corregida
...
print(classification_corregida, 'classification corregida')


Export.image.toAsset({
    'image': classification_corregida,
...
// Exportar a Google Drive
  if(param.exportOpcion.exportClasifToDrive){
    Export.image.toDrive({
      image: classification_corregida.toInt8(),
...
if(param.exportOpcion.exportEstadistica){
  getAreas(classification_corregida, regions)
}
...
var areasPorAño0 = year.map(function(año) {
  ...
  var area0 = enmascararValor(classification_input, nombreBanda).reduceRegion({
    reducer: ee.Reducer.sum(),
...
var areasPorAño = year.map(function(año) {
  var nombreBanda = ee.String('classification_').cat(ee.Number(año).int().format());
  var area = enmascararValor(classification_corregida, nombreBanda).reduceRegion({
    reducer: ee.Reducer.sum(),
    geometry: regions.geometry(),
    scale: 250, // Ajusta según la resolución de tu imagen
    maxPixels: 1e9
